...
(function($) {
    $.fn.extend({
        ...
        jdigiclock: function(options) {
            ...
            return this.each(function() {
                ...
                $this.minutesOffset = o.minutesOffset;
    ...
    $.fn.getTime = function(el) {
    	// TODO Change the time to display here!
//    	var now = new Date("December 8, 2011 22:43:00");
    	var old = new Date();

    	var startHour = parseInt(el.startHour) ? el.startHour : old.getHours();
    	var startMinutes = parseInt(el.startMinute) ? el.startMinute : old.getMinutes();

        var now = new Date();
        now.setHours(startHour);
    	now.setMinutes(startMinutes + parseInt(el.minutesOffset) + (old.getMinutes() % 5));
